...
export default function Index() {
  ...
  const getTextStyle = () => {
    ...
    // Calculate preview font size based on container scaling
...
<to_replace>  const createImageWithText = useCallback(async (imageFile, text) => {
  ...
  }, [imageDimensions, textBox, detectLanguage, getExportDimensions, exportFormat]);</to_replace>
<new_content>  const createImageWithText = useCallback(async (imageFile, text) => {
    return new Promise((resolve) => {
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      ...
      img.onload = () => {
        ...
        // Use the same font size calculation as preview
        const fontSize = textBox.style.fontSize * scaleX;
        
        // Set font properties exactly as in preview
        ...
        // Set text direction for Hebrew/RTL support
        ...
        // Apply stroke if specified (outside stroke effect)
        if (textBox.style.strokeWidth > 0) {
          ctx.strokeStyle = textBox.style.strokeColor;
                    ...
                    >
                      {imageUrl ? (
                        <>
                          ...
                          >
                            <div className="w-full h-full flex items-center justify-center p-1 sm:p-2" style={getTextStyle()}>
                              {textBox.text}
                            </div>

                            {!isDragging && !isResizing && (
                              <>
                                <div className="absolute -top-1 -left-1 w-4 h-4 sm:w-3 sm:h-3 bg-blue-500 border border-white rounded-full cursor-nw-resize hover:bg-blue-600 touch-manipulation" onMouseDown={(e) => handleMouseDown(e, 'resize', 'nw')} onTouchStart={(e) => handleTouchStart(e, 'resize', 'nw')} />
                                <div className="absolute -top-1 -right-1 w-4 h-4 sm:w-3 sm:h-3 bg-blue-500 border border-white rounded-full cursor-ne-resize hover:bg-blue-600 touch-manipulation" onMouseDown={(e) => handleMouseDown(e, 'resize', 'ne')} onTouchStart={(e) => handleTouchStart(e, 'resize', 'ne')} />
                                <div className="absolute -bottom-1 -left-1 w-4 h-4 sm:w-3 sm:h-3 bg-blue-500 border border-white rounded-full cursor-sw-resize hover:bg-blue-600 touch-manipulation" onMouseDown={(e) => handleMouseDown(e, 'resize', 'sw')} onTouchStart={(e) => handleTouchStart(e, 'resize', 'sw')} />
                                <div className="absolute -bottom-1 -right-1 w-4 h-4 sm:w-3 sm:h-3 bg-blue-500 border border-white rounded-full cursor-se-resize hover:bg-blue-600 touch-manipulation" onMouseDown={(e) => handleMouseDown(e, 'resize', 'se')} onTouchStart={(e) => handleTouchStart(e, 'resize', 'se')} />
                              </>
                            )}
                          </div>
                        </>
