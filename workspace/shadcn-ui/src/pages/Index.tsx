...
import { Upload, Download, FileText, Image as ImageIcon, Settings, Archive } from 'lucide-react';
...
export default function Index() {
  ...
  const getTextStyle = () => {
    ...
    // Hebrew system fonts with proper fallbacks
    const getSystemFontFamily = (font) => {
    ...
    return {
      ...
      color,
      fontFamily: getSystemFontFamily(fontFamily),
      textAlign: align,
      direction,
      lineHeight: '1.2',
      wordWrap: 'break-word',
  ...
  const createImageWithText = useCallback(async (imageFile, text) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        // Hebrew system fonts with proper fallbacks for canvas
        const getCanvasFontFamily = (font) => {
          const hebrewFonts = ['David', 'Narkisim', 'Rod', 'Miriam', 'Gisha', 'Tahoma'];
          if (hebrewFonts.includes(font)) {
            return `${fontSize}px "${font}", "Arial Hebrew", "Times New Roman", Arial, sans-serif`;
          }
          // English fonts
          return font.includes(' ') 
            ? `${fontSize}px "${font}", Arial, sans-serif`
            : `${fontSize}px ${font}, Arial, sans-serif`;
        };
        
        ctx.font = getCanvasFontFamily(fontFamily);
  ...
  const downloadAllAsZip = async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `bulk_text_overlay_${resolution.value}dpi_${processedImages.length}images.zip`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      
      toast.success(`Downloaded ZIP with ${processedImages.length} images`);
    } catch (error) {
